...
We can apply a few checks to verify whether or not the results estimated above are reasonable.
...
Note that this returns `NA`s for `{r} sum(is.na(water_system_demographics_interpolate_pw_count$population_total_count))` water systems, which generally appear to be relatively smaller systems. 

@tbl-interpolate-pw-compare shows a comparison of the system populations estimated using `interpolate_pw` and the reported system populations.
