...
This document provides an example of how to use tools available from the [R programming language](https://www.R-project.org/) [@R] to estimate characteristics of any given *target* spatial area(s) (e.g., neighborhoods, project boundaries, water supplier service areas, etc.) based on data from a *source* dataset containing the characteristic data of interest (e.g., census data, CalEnviroScreen scores, etc.), especially when the boundaries of the *source* and *target* areas overlap but don't necessarily align with each other. It also provides some brief background on the various types of data available from the U.S Census Bureau, and links to a few places to find more in-depth information.
...
The American Community Survey (ACS) provides a much larger array of demographic information than the Decennial Census, and is updated more frequently. The ACS is based on a sample of the population (rather than a count of the entire population, as in the Decennial Census), so it represents estimated values rather than precise counts; therefore, each data point is available as an estimate (typically labeled with an "E" in census variable codes, which are discussed in @sec-census-variables ) along with an associated margin of error (typically labeled with "M" or "MOE" in census variable codes) around its estimated value. The MOEs for ACS data are typically provided at a 90% confidence level -- to calculate the 90% confidence interval for an estimate, add the MOE to the estimated value to get the upper bound of the confidence interval, and subtract the MOE from the estimate to get the lower bound of the confidence interval (for more information see [here](https://www.census.gov/content/dam/Census/library/publications/2018/acs/acs_general_handbook_2018_ch07.pdf)). Note that it's possible to calculate MOEs for some types of derived estimates of census data, such as aggregating data across multiple census units or calculating proportions and percentages (see [here](https://www.census.gov/content/dam/Census/library/publications/2018/acs/acs_general_handbook_2018_ch08.pdf) for more information); however, it may be difficult or not possible to calculate MOEs for some more complicated types of derived estimates (like some of the aggregation methods described below).
...
### Create Spatial Filter {#sec-census-access-create-filter}
...
2.  Using the estimated count data (populations, households, etc), compute weighted values for remaining variables, with the associated count data as a weighting factors – e.g., population-weighted values for population based data, or household-weighted values for household-based data. These variables are typically referred to as 'intensive' variables.
...
3.  Aggregate interpolated values at the water system level, summing the count data for variables computed in step 1, and computing weighted means for count-weighted variables computed in step 2.

### Prepare Census Data {#sec-estimate-demographics-prep-census-data}
...
# commenting this out to retain all data - can combine later if needed

# We can also do some other transformations to simplify the data. For example, we could combine the 'other' and 'multiple' racial/ethnic groupings into one 'other or multiple' racial/ethnic group.
...
### Interpolation Step 1: Estimate Data for Count (Extensive) Variables with Areal Interpolation {#sec-areal-interp}
...
Water system `{r} str_to_title(system_plot)` (filled polygon), boundaries of overlapping census units (grey), and clipped portions of census units (light blue) that will be used to estimate water system demographics.
:::
...
### Interpolation Step 2: Estimate Weighted Values for Remaining (Intensive) Variables Based on Interpolated Counts {#sec-pop-interp}
...
::: {#cau-median-income .callout-caution}
...
### Interpolation Step 3: Aggregate by Water System {#sec-estimate-demographics-aggregate-by-system}
...
Since computing a weighted mean for the median household income may be somewhat inaccurate (as noted above in @cau-median-income), it may also be worth calculating a grouped median household income based on the income bracket data:
...
### View Results {#sec-results-clean}
...
@tbl-water-sys-demographics-rev shows the cleaned and re-formatted dataset (these results are saved locally in tabular and spatial format in @sec-results-save below).
...
#### Save Tabular Dataset {#sec-results-save-tabular}
...
#### Save Spatial Dataset {#sec-results-save-spatial}
...
This section presents some visualizations of the estimated water system demographics computed above. However, for the most part, identical visualizations could be produced for results from any of the methods described below in @sec-alternative-methods (or other methods not described in this document) by replacing the `water_system_demographics` or `water_system_demographics_long` objects with the results from those methods.

The map in @fig-results-map below shows a summary of all of the estimated demographic variables for each water system:
...
::: callout-warning
For small / medium sized target areas (small water systems), count data estimated using this method may be highly unreliable (since entire census units are used). In those cases, it's likely that only the estimated rates / percentages may be useful, but it may be worth considering whether it's worth making any estimates for those systems based on census data alone. See @sec-alternative-simplified-investigate and @sec-small-area-estimates for some further exploration of the issues when dealing with estimates for small areas from census data alone.
:::
...
#### Compute Demographic Estimates {#sec-alternative-simplified-calculate}
...
##### Calculated Count-Weighted Values

Next, compute weighted values for remaining variables, using estimated count data from the previous step (population or households) as weighting factors (as described above in @sec-pop-interp):
...
##### Aggregate by Water System

Next, aggregate the data for each water system (as described above in @sec-estimate-demographics-aggregate-by-system) -- do this by summing all of the count-based variables, and calculating weighted averages for all remaining count-weighted variables.
...
@tbl-water-sys-demographics-simplified shows the estimated demographics for each water system using the simplified interpolation method:

```{r}
#| message: false
#| warning: false
#| label: tbl-water-sys-demographics-simplified
#| tbl-cap: "Estimated Water System Demographics - Simplified Method"
#| tbl-cap-location: top
#| code-fold: true
...
#### Investigate / Check Assumptions {#sec-alternative-simplified-investigate}
...
Water system `{r} str_to_title(system_plot_small_2)` (light blue fill / black border), boundaries of census units (dark grey fill / blue border) used to estimate water system demographics for the simplified approach, and boundaries of census units overlapping the water system but not included in the demographic estimates (light grey fill).
:::
...
According to the documentation for the [`interpolate_pw`](https://walker-data.com/tidycensus/reference/interpolate_pw.html) function, the approach it implements is based on Esri's data apportionment algorithm -- more information about that can be found [here](https://pro.arcgis.com/en/pro-app/latest/help/analysis/business-analyst/data-apportionment-and-layers.htm) and [here](https://developers.arcgis.com/rest/geoenrichment/api-reference/data-apportionment.htm).

::: callout-warning
Margins of error (MOEs) for estimated values cannot be calculated directly using the `interpolate_pw` function (and may be difficult to calculate at all) -- the  [`interpolate_pw`](https://walker-data.com/tidycensus/reference/interpolate_pw.html) documentation states: *Margins of error in the ACS will not be transferred correctly with this function, so please use with caution*
:::

One drawback of using this approach is that it may not work well in cases where the overall area covered by the target area is significantly smaller than the area covered by the source dataset -- for example, small water systems are often not given an estimated value using this method and `NA`s are returned for many of those areas (even if `NA` values are removed from the source data first). More research / feedback may be needed on how applicable this approach may be for certain use cases. It may also be somewhat difficult to explain and intrepret the results.

#### Interpolate 

For these computations, we can use the ACS data that was accessed above in @sec-census-access-acs and transformed in @sec-estimate-demographics-prep-census-data, and the decennial census data that was accessed above in @sec-census-access-decennial.
...
water_system_demographics_interpolate_pw_extensive_hh <- 
    interpolate_pw(
        from = census_data_acs %>%
            filter(!is.na(households_count)) %>% 
...
# population weighted variables ----
water_system_demographics_interpolate_pw_intensive_pop <- 
    interpolate_pw(
        from = census_data_acs %>%
            filter(!is.na(population_total_count)) %>% 
...
# household weighted variables ----
water_system_demographics_interpolate_pw_intensive_hh <- 
    interpolate_pw(
        from = census_data_acs %>%
            filter(!is.na(households_count)) %>% 
...
#### Compute Additional Aggregated Data
...
Note that this process returns `NA`s for `{r} sum(is.na(water_system_demographics_interpolate_pw$population_total_count))` water systems, which generally appear to be relatively smaller systems.
...
@tbl-water-sys-demographics-interpolate-pw shows all demographic variables estimated using the population weighted areal interpolation approach with the tidycensus `interpolate_pw` function.
...
### Modified Population / Household Weighted Areal Interpolation {#sec-alternative-modified-pw-areal}
...
::: callout-note
While this method attempts to estimate the distribution of population and households within census block groups for ACS data, it makes the simplifying assumption that population / household characteristics are consistent/homogeneous across the populations / households in each block group. So, this method can't account for clusters of populations or households within block groups that have demographic characteristics that aren't representative of the entire block group, such as clusters of high or low income households or clusters of populations of a given racial/ethnic group.

Also, it would be possible to estimate distributions of racial/ethnic groups within block groups -- because that data is available at the block level from the decennial census -- but this method avoids doing that, because other population variables (income, education) are likely correlated to some degree with race/ethnicity, and we can't estimate the distributions of those other variables (so using estimated racial/ethnic group distributions could distort any analysis of those correlations).
:::

#### Estimate Population / Household Distributions Within Block Groups {#sec-alternative-estimate-distributions}

This section uses decennial census block and block group data to compute weights that represent the distribution of population and households by census block within each census block group. For any given block, this weight is equal to the population count or number of households in that block divided by the population count or number of households in the block group which that block is contained within. In other words, it's the portion of the block group's population / households contained within that block.

To start, get tabular data on total population and number of households for all blocks and block groups in the counties containing the water systems we're assessing:
...
df_decennial_blocks <- get_decennial(geography = 'block', 
                                     state = 'CA', 
                                     county = counties_list,
                                     year = decennial_year,
                                     variables = c('population_block_count' = 'P2_001N', # total population  
                                                   'households_block_count' = 'H1_002N'), # total households (occupied)
                                     output = 'wide', 
                                     geometry = FALSE,
                                     cache_table = TRUE)

df_decennial_blockgroups <- get_decennial(geography = 'block group', 
                                          state = 'CA', 
                                          county = counties_list,
                                          year = decennial_year,
                                          variables = c('population_blockgroup_count' = 'P2_001N', # total population  
                                                        'households_blockgroup_count' = 'H1_002N'), # total households (occupied)
                                          output = 'wide', 
                                          geometry = FALSE,
                                          cache_table = TRUE)
```

Next, compute population and household weights for each block (as the portion of each block's population or households relative to the total population or households in its respective block group):

```{r}
#| message: false
#| warning: false

# get block group GEOIDs for blocks ----
df_decennial_blocks <- df_decennial_blocks %>% 
    rename(block_geoid = GEOID,
           block_name = NAME) %>%
    mutate(blockgroup_geoid = str_sub(string = block_geoid, 
                                      start = 1, 
                                      end = 12))
# glimpse(df_decennial_blocks)

# clean block group data ----
df_decennial_blockgroups <- df_decennial_blockgroups %>% 
    rename(blockgroup_geoid = GEOID,
           blockgroup_name = NAME)
# glimpse(df_decennial_blockgroups)

# compute block population / household weights ----
## join block group data to blocks ----
df_decennial_block_weights <- df_decennial_blocks %>% 
    left_join(df_decennial_blockgroups, by = 'blockgroup_geoid')

## compute weights ----
df_decennial_block_weights <- df_decennial_block_weights %>% 
    mutate(block_population_decennial_weight = ifelse(population_blockgroup_count == 0, # a few block groups have zero population
                                            0,
                                            population_block_count / population_blockgroup_count),
           block_household_decennial_weight = ifelse(households_blockgroup_count == 0, # a few block groups have no households
                                            0, 
                                            households_block_count / households_blockgroup_count))
# glimpse(df_decennial_block_weights)

# ## check - make sure weights add to 1 for all block groups ----
# df_decennial_block_weights_check <- df_decennial_block_weights %>%
#     group_by(blockgroup_geoid) %>%
#     summarize(block_population_decennial_weight_sum = round(sum(block_population_decennial_weight), 4),
#               block_household_decennial_weight_sum = round(sum(block_household_decennial_weight), 4)) %>% 
#     ungroup()
# # glimpse(df_decennial_block_weights_check)
# df_decennial_block_weights_check %>% count(block_population_decennial_weight_sum)
# df_decennial_block_weights_check %>% count(block_household_decennial_weight_sum)

# clean block weight dataset ----
## (will make it easier to apply / less confusing for subsequent calculations)

df_decennial_block_weights <- df_decennial_block_weights %>% 
    select(block_geoid, 
           block_population_decennial_weight, 
           block_household_decennial_weight, 
           blockgroup_geoid)
```

Then, using the estimated population and household distributions computed above, transfer the ACS data at the block group level to census blocks (note that this uses the ACS data that was accessed above in @sec-census-access-acs and transformed / re-formatted in @sec-estimate-demographics-prep-census-data):
...
# prepare ACS datset ----
df_acs_data <- census_data_acs %>% 
    st_drop_geometry() %>% 
    # select(-poverty_rate_pct_calc_census_unit)
    rename(blockgroup_geoid = GEOID,
           blockgroup_name = NAME)
# glimpse(df_acs_data)

# join ACS data to block weight data ----
# glimpse(df_decennial_block_weights)
df_acs_data_estimated_blocks <- df_decennial_block_weights %>% 
    inner_join(df_acs_data, 
               by = 'blockgroup_geoid')
# glimpse(df_acs_data_estimated_blocks)

# scale ACS block group level data to block level data ----
df_acs_data_estimated_blocks <- df_acs_data_estimated_blocks %>% 
    mutate(across(
        .cols = starts_with('population_') & ends_with('_count'),
        .fns = ~ .x * block_population_decennial_weight
    )) %>% 
    mutate(across(
        .cols = starts_with('poverty_') & ends_with('_count'),
        .fns = ~ .x * block_population_decennial_weight
    )) %>%
    mutate(across(
        .cols = starts_with('households_') & ends_with('_count'),
        .fns = ~ .x * block_household_decennial_weight
    ))

# transfer estimated ACS block-level data to block spatial data ----

## get block level geometry ----
## NOTE: block-level geometry is already stored in the census_data_decennial object, but that 
## was filtered only for blocks that overlap the water systems - some blocks included in block 
## groups overlapping the water systems may not be included, so for consistency get all of those
## blocks

### create spatial filter ----
acs_blockgroup_filter <- census_data_acs %>% 
    st_union() %>% 
    st_as_sf()

### get blocks ----
## NOTE: the tigris::blocks() function returns fields with the population (POP20) and number of 
## households (HOUSING20) for each block. The POP20 variable has the same data as the population data 
## accessed above (decennial variable: P2_001N). But, the HOUSING20 variable represents the total of 
## occupied + vacant households (decennial variable: H1_001N); however, we're just using the number of 
## occupied households (decennial variable: H1_002N) (because that's in line with the denominator for 
## most ACS variables, like income brackets)

census_blocks <- blocks(state = 'CA', 
                        county = counties_list, 
                        year = acs_year,
                        filter_by = acs_blockgroup_filter) %>% 
    st_transform(crs_projected)

### filter for blocks contained in block groups included in the ACS data ----
census_blocks <- census_blocks %>% 
    select(block_geoid = GEOID20, 
           block_name = NAME20, 
    ...
    filter(blockgroup_geoid %in% df_acs_data$blockgroup_geoid)

### rename and remove census_blocks object ----
### (to avoid keeping two copies of the census_blocks object - may be large) 
sf_acs_data_estimated_blocks <- census_blocks
rm(census_blocks)


## join estimated ACS block-level data to block spatial data ----
sf_acs_data_estimated_blocks <- sf_acs_data_estimated_blocks %>% 
                   select(block_geoid,
                          block_name,
                          geometry) %>% 
    inner_join(df_acs_data_estimated_blocks, # rename GEOID field to make consistent
               by = 'block_geoid') %>% 
    relocate(geometry, .after = last_col())
# glimpse(sf_acs_data_estimated_blocks)


# # check ----
# ## make sure estimated (scaled) ACS block-level data matches original block group ACS data
# 
# ## summarize the estimated data at the block group level (sum block count data for each block group) ----
# check_acs_data_estimated_blocks <- sf_acs_data_estimated_blocks %>%
#     st_drop_geometry() %>% 
#     group_by(blockgroup_geoid) %>%
#     summarize(
#         across(
...
#             .fns = ~ sum(.)
#         )) %>%
#     rename_with(.cols = starts_with('population_') & ends_with('_count'),
#                 .fn =  ~ str_replace(., pattern = 'population_', replacement = 'population_block_sum_')) %>%
#     rename_with(.cols = starts_with('poverty_') & ends_with('_count'),
#                 .fn =  ~ str_replace(., pattern = 'poverty_', replacement = 'poverty_block_sum_')) %>%
#     rename_with(.cols = starts_with('households_') & ends_with('_count'),
#                 .fn =  ~ str_replace(., pattern = 'households_', replacement = 'households_block_sum_'))
# 
# ## join original ACS data to summarized estimated data ----
# check_acs_data_estimated_blocks <- check_acs_data_estimated_blocks %>%
#     left_join(df_acs_data %>% select(-blockgroup_name),
#               by = 'blockgroup_geoid')
# # glimpse(check_acs_data_estimated_blocks)
# 
# ## compare - this should be zero for any variable chosen ----
# check_acs_data_estimated_blocks %>%
#     select(acs_original = population_block_sum_asian_count, # population_total_count # households_count # population_block_sum_asian_count
#            acs_estimated_blocks_sum = population_asian_count) %>% # population_block_sum_total_count # households_block_sum_count # population_asian_count
#     mutate(difference = acs_original - acs_estimated_blocks_sum) %>%
#     pull(difference) %>%
#     round(2) %>%
#     range()
```

#### Compute Demographic Estimates for Target Areas

Now, we can apply the same steps as described above in @sec-estimate-demographics-method-description to compute demographics for each water system.

##### Interpolation Step 1: Estimate Data for Count (Extensive) Variables with Areal Interpolation {#sec-areal-interp-modified}

First, clip the estimated ACS block-level data to the water system boundaries (as described in @sec-areal-interp-modified) to determine what portion of each block overlaps each water system:

```{r}
#| message: false
#| warning: false

acs_estimated_blocks_clip <- sf_acs_data_estimated_blocks %>% 
    mutate(census_unit_area = st_area(.)) %>% 
    st_intersection(water_systems_sac) %>% 
    mutate(clipped_area = st_area(.)) %>% 
    mutate(areal_weight_factor = drop_units(clipped_area / census_unit_area))
...
@fig-map-clipped-polygons-modified-method shows a plot of the census units clipped to the `{r} str_to_title(system_plot_small_2)` water system, along with the original/complete census units. Note that you can toggle layers on and off (and change their order of appearance) using the layers button in the upper left part of the map (below the zoom buttons).

::: {#fig-map-clipped-polygons-modified-method}
```{r}
#| message: false
#| warning: false
#| code-fold: true
...
mapview(water_systems_sac %>% 
        ...
        layer.name = 'Water System', 
        legend = FALSE) + 
    mapview(sf_acs_data_estimated_blocks %>% 
                st_filter(census_data_acs %>% 
                              st_filter(water_systems_sac %>% 
                                            filter(water_system_name == system_plot_small_2))), 
            alpha.regions = 0.15, 
            col.regions = 'grey', 
            ...
            lwd = 0.8, 
            label = 'block_name',  
            layer.name = 'ACS Blocks (Estimated) - Full', 
            legend = FALSE, 
            popup = popupTable(sf_acs_data_estimated_blocks,
                                      zcol = c('block_name',
                                               'population_total_count',
                                               'households_count'))) +
    mapview(acs_estimated_blocks_clip %>% 
                filter(water_system_name == system_plot_small_2),
            alpha.regions = 0, 
            color = 'cyan', 
            lwd = 1.3, 
            label = 'block_name',  
            layer.name = 'ACS Blocks (Estimated) - Clipped', 
            legend = FALSE,
            popup = popupTable(acs_estimated_blocks_clip,
                                      zcol = c('block_name',
                                               'population_total_count',
                                               'households_count'))) +
        mapview(census_data_acs %>% 
            ...
            lwd = 1.5, 
            label = 'NAME',  
            layer.name = 'ACS Block Groups - Full', 
            legend = FALSE,
            popup = popupTable(census_data_acs,
                                      zcol = c('NAME',
                                               'population_total_count',
                                               'households_count')))
```

Water system `{r} str_to_title(system_plot_small_2)` (filled polygon), boundaries of overlapping census block groups (black boundaries / grey fill), overlapping census blocks (red boundaries / grey fill) which ACS data was transferred to (using estimated estimated population distributions from decennial data), and clipped portions of census blocks (light blue) that will be used to estimate water system demographics.
:::

Next, compute area-weighted counts for the portions of the census blocks that overlap each water system boundary, based on the estimated block-level ACS data:

```{r}
#| message: false
#| warning: false

acs_estimated_blocks_interpolate <- acs_estimated_blocks_clip %>% 
    mutate(
        across(
            .cols = ends_with('_count'),
            .fns = ~ .x * areal_weight_factor
...
##### Interpolation Step 2: Estimate Weighted Values for Remaining (Intensive) Variables Based on Interpolated Counts {#sec-pop-interp-modified}

Next, compute count-weighted values for remaining variables (as described in @sec-pop-interp above).

```{r}
#| message: false
#| warning: false

acs_estimated_blocks_interpolate <- acs_estimated_blocks_interpolate %>% 
    mutate(average_household_size_weighted = average_household_size * households_count,
           median_household_income_weighted = median_household_income * households_count,
           per_capita_income_weighted = per_capita_income * population_total_count)
```

##### Interpolation Step 3: Aggregate by Water System {#sec-estimate-demographics-aggregate-by-system-modified}

Next, combine the variables calculated above to produce estimated demographics for each water system, by summing all of the count-based variables computed in step 1 above, and calculating weighted means for all count-weighted variables computed in step 2 above (as described in @sec-estimate-demographics-aggregate-by-system above).
...
# aggregate ----
water_system_demographics_acs_estimated_blocks <- acs_estimated_blocks_interpolate %>% 
    mutate(
        average_household_size_denominator = if_else(
            is.na(average_household_size), 
            0, 
...
# round count data to nearest whole number
water_system_demographics_acs_estimated_blocks <- water_system_demographics_acs_estimated_blocks %>%
    mutate(
        across(
            .cols = ends_with('_count'),
            .fns = ~ round(.x, 0)
...
#### View Results {#sec-results-clean-modified-interp}

@tbl-water-sys-demographics-rev-modified-interp shows the estimated demographics for each water system using the modified interpolation method:

```{r}
#| message: false
#| warning: false
#| label: tbl-water-sys-demographics-rev-modified-interp
#| tbl-cap: "Estimated Water System Demographics"
#| tbl-cap-location: top
#| code-fold: true
...
water_system_demographics_acs_estimated_blocks %>%
    st_drop_geometry() %>% 
    mutate(across(
        .cols = ends_with('_percent'),
        .fns = ~ pct_format(. / 100))
...
````
<!-- 
```` !!!!!!!!!! COMMENTED SECTION - END (DELETE THESE 4 LINES) !!!!!!!!!! 
-->



### Method Comparison {#sec-alternative-method-comparison}
...
## Detailed Population Estimates with Decennial Data {#sec-detailed-pop-estimates}
...
### Estimate Populations with Areal Interpolation
...
@tbl-water-sys-demographics-areal-detailed shows all demographic variables estimated using the block-level data from the `r decennial_year` Decennial Census.

```{r}
#| message: false
#| warning: false
#| label: tbl-water-sys-demographics-areal-detailed
#| tbl-cap: "Estimated Water System Demographics - Areal Interpolation with Decennial Block Group Data"
#| tbl-cap-location: top
#| code-fold: true
...
## Small / Rural Area Estimate Issues & Considerations {#sec-small-area-estimates}
...
## Tribal Data {#sec-tribal-data}

::: callout-warning
This section is in progress.
:::

## Working with Other Source Data {#sec-other-sources}
