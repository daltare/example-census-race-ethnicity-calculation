...
@fig-suppliers-census-map shows the datasets that we'll use below to compute water system demographics; for this example we'll use the `r acs_year` 5-year ACS dataset for the `{r} system_plot` system (plotting all systems tends to be slow in this format). Note that you can toggle layers on and off (and change their order of appearance) using the layers button in the upper left part of the map (below the zoom buttons).
...
There are multiple ways this estimation can be done. For this example, we'll employ a two step strategy:

1.  To estimate population (total and by demographic group) and other count data (population above / below poverty rate, households by income bracket, etc.) for overlapping census units, apply an areal interpolation (essentially an area weighted average) to estimate how much of the census unit's count applies to the associated water service area. For example, for a census unit that partially overlaps a service area, only a fraction of its count for a given variable will be applied to that service area; for a census unit that completely overlaps a service area, the full count for that variable will be applied to the service area.
...
2.  Using the estimated count data (populations, households, etc), compute weighted averages for variables that describe those populations, using the associated count data as a weighting factor (e.g., population-weighted averages for population based data, or household-weighted averages for household-based data). This method was chosen because, from the population / household count data, we know that population densities differ between census units. So, since we have a reasonable estimate of population within each census unit, using a population or household weighted average likely will yield more accurate results than a simple area-weighted average for these variables.

::: callout-caution
Areal interpolation, based on area weighted averaging, is useful for estimating count data for overlapping polygons - like population size or number of households - since we generally have no other information about how population density varies within the source polygons (unless significantly more effort is invested, such as looking at aerial imagery data).

While it's possible to also use areal interpolation to estimate variables that are associated with / describe the estimated count data (generally referred to as 'intensive' variables) - like average household size, per capita income, median household income, etc - it may be be more appropriate to use the corresponding estimated count data (rather than area) as a weighting factor. For example, for per capita income, we can use the estimated population counts to produce a population weighted average per capita income (rather than an area weighted average per capita income). This is the approach taken here.
:::

::: callout-tip
There may be cases where it could be useful or necessary to use more granular block-level population data from the decennial census to estimate population densities and distributions within larger census units, like block groups and tracts. This could especially be the case when estimating characteristics for small areas in rural environments. See @sec-pop-weighted-interpolation for more information.
:::
...
As noted above, it's also possible to use pre-built functions for areal interpolation. Since we're using a two-step process which also implements population weighted averaging we're not using them directly in this example, but they can be a useful check of our computed count data.
...
```{r}
# ## sf package
# z <- st_interpolate_aw(x = census_data_acs %>% select(population_total_count, poverty_below_count), 
#                        to = water_systems_sac, 
#                        extensive = TRUE)
# View(z %>% arrange(desc(population_total_count)))
# 
# ## areal package
# zz <- aw_interpolate(water_systems_sac, 
#                tid = WATER_SY_1, 
#                source = census_data_acs, 
#                sid = GEOID, 
#                weight = 'total', 
#                extensive = c('population_total_count', 'poverty_below_count'))
# View(zz %>% arrange(desc(population_total_count)))
# View(water_system_demographics %>% arrange(desc(population_total_count)))
# 
# sum(z$population_total_count) == sum(water_system_demographics$population_total_count)
# sum(zz$population_total_count) == sum(water_system_demographics$population_total_count)
```
...
### Summarize Demographics by Water System
...
water_system_demographics <- water_system_demographics %>% 
    mutate(poverty_rate_pct_calc_system = 100 * poverty_below_count / poverty_total_assessed_count, 
           .after = poverty_above_count)
```

Compute 25k income brackets
...
Calculate grouped median household income....

```{r}

```

Compute other percentages (demographic groups, % households by housing cost, % households below income thresholds)
...
```{r}
# glimpse(water_systems_sac_demographics[,1:20])
```
...
For simplicity, this section will focus on presenting estimated demographics for the largest water suppliers in the Sacramento county region. Results for small water systems may not be very accurate and should be used with some caution (because the water system may encompass only a small portion of one or a few census units, and the entire census unit(s) may not be representative of the small portion(s)), especially those in rural environments (where population densities are lower, population centers tend to be spread out, and census units tend to be larger).

## Check - Estimated vs Reported Population Estimates
...
### Population Weighted Interpolation {#sec-interpolate_pw}

Can also use the [`interpolate_pw`](https://walker-data.com/tidycensus/reference/interpolate_pw.html) function from the `tidycensus` package - uses a somewhat different methodology than applied above in @sec-pop-interp.

Use 2020 census population by census block from above (not strictly necessary, but this is the most spatially granular census population data available).

Note that some water systems may not get an estimated value using this method (even if `NA` values are removed from the source data) -- may need to check which type of boundary dataset is used (i.e., tigris with cb = FALSE or TRUE).
...
## Working with Other Source Datasets
