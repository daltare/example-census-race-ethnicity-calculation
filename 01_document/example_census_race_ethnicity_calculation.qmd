...
water_system_demographics <- water_system_demographics %>%
    mutate(households_all_housing_costs_over30pct_percent = 
               ...
               households_count, 
           .after = households_rent_over50pct_count) %>% 
...
census_data_interpolate <- census_data_interpolate %>%      
    mutate(households_all_housing_costs_over30pct_percent = 
               100 * (households_mortgage_over30pct_count +                           
                          households_no_mortgage_over30pct_count +                           
                          households_rent_over30pct_count) / households_count, 
           .after = households_rent_over50pct_count) %>%      
    mutate(households_all_housing_costs_over50pct_percent = 
               100 * (households_mortgage_over50pct_count +                            
                          households_no_mortgage_over50pct_count +                           
                          households_rent_over50pct_count) / households_count,
           .after = households_all_housing_costs_over30pct_percent)
...
# create classes
biscale_data_system <- bi_class(census_data_clip %>% 
                                    filter(water_system_name == system_plot) %>% 
