...
if (sum(!st_is_valid(water_systems_sac_alternative)) > 0) {
    water_systems_sac <- st_make_valid(water_systems_sac_alternative)
}
```
...
water_system_demographics_simplified_method <- water_system_demographics_simplified_method %>%
    mutate(
        across(
            ...
            .names = "{str_replace(.col, '_countE', '_percent')}"
        ),
...
#### Interpolate Extensive (Count) Variables
...
Then, using the estimated population and household distributions computed above, transfer the ACS data at the block group level to census blocks (note that this uses the ACS data that was transformed / re-formatted above in @sec-estimate-demographics-prep-census-data):
