...
::: callout-note
For comments, suggestions, corrections, or questions on anything below, contact [david.altare\@waterboards.ca.gov](mailto:david.altare@waterboards.ca.gov), or [file an issue](https://github.com/daltare/example-census-race-ethnicity-calculation/issues) on github.
:::
...
::: callout-caution
Census boundaries can change over time. Commonly used statistical units like tracts, block groups, blocks, etc. tend to be revised every 10 years (with the decennial census), so it's important to use a census boundary dataset that matches the version of the census demographic data you're retrieving; otherwise, the demographic data may not be referring to the geographic areas in your boundary dataset. In some cases, census units can be split or merged, so a census unit that exists in a given year of the census data may not exist at all in a differen't year's dataset.
...
Note that for this dataset we've also included statements to get additional socio-economic data, such as counts of population below and above the poverty line, median household income, and income profile data (income by bracket).
...
#### Revise Calculation of Intensive Variables {#sec-revise-interp-med-hh-inc}

::: callout-caution
As noted above, using interpolated results for the variables in the intensive list below may not be the best approach.

Especially for median household income an areal weighted interpolation may not be appropriate, because aggregating multiple census units changes the distribution of household income, so the resulting value (weighted average of medians) may not reflect the true median very well. It may be more appropriate to use the counts in each income bracket to estimate a median income and/or present the income distribution rather than a single value.
:::

Note that for `r sum(is.na(water_systems_sac_demographics$median_household_income))` water systems, the estimated median household income is missing. That's because the median household income for is missing for some of the census block groups we retrieved above (a total of `r sum(is.na(census_data_filter$median_household_income))` of the overlapping census block groups from the `r acs_year` 5-year ACS dataset are missing median household income data).
...
water_systems_sac_demographics_hh_inc <- water_systems_sac %>% 
    aw_interpolate(tid = WATER_SY_1, 
                   source = census_data_filter %>% 
                       filter(!is.na(median_household_income)), # drop census units with missing income data
                   sid = GEOID, 
                   # weight = 'sum',
                   output = 'sf', 
                   intensive = c('median_household_income')
    )
```

The re-calculated median household income estimates have `r sum(is.na(water_systems_sac_demographics_hh_inc$median_household_income))` missing value(s), and we can replace the original median household income estimates with these re-calculated values:

```{r}
water_systems_sac_demographics <- water_systems_sac_demographics %>% 
    mutate(median_household_income = water_systems_sac_demographics_hh_inc$median_household_income)
...
Finally, just as a check, add a column (which we'll call `population_percent_difference`) to compute the difference between the estimated total population (in the `population_est_total` field) and the total population listed in the `POPULATION` field (the reported value from the water system dataset).

```{r}
water_systems_sac_demographics <- water_systems_sac_demographics %>% 
    mutate(population_percent_difference = 
               round((population_est_total - POPULATION) / POPULATION * 100, 2))

# rearrange fields to make order more logical
water_systems_sac_demographics <- water_systems_sac_demographics %>% 
    relocate(population_est_total, .after = 'POPULATION') %>% 
    relocate(population_percent_difference, .after = 'population_est_total') %>% 
    relocate(median_household_income_est, geometry, .after = last_col()) %>% 
    select(-percent_est_total) # drop the percent_est_total field - it's always just "1"
```
...
water_systems_sac_demographics_long <- water_systems_sac_demographics %>% 
    select(WATER_SY_1, starts_with('percent_est_')) %>% # select only the fields with percentages, and the water system name/id
    # convert to long format
    pivot_longer(cols = starts_with('percent_est_'), names_to = 'metric') %>% 
    # format the metric field to make titles
    mutate(metric = metric %>% 
               str_remove_all(pattern = 'percent_est_') %>% 
...
plot_results_non_white <- ggplot() +
    geom_sf(data = water_systems_sac_demographics %>% 
                mutate(percent_est_non_white = 100 - percent_est_white), # compute non-white portion of population for each water system
            aes(fill = percent_est_non_white), 
            color = 'black') +
    scale_fill_distiller(palette = 'Blues', 
                         direction = 1) +
    geom_sf(data = counties_ca %>% 
...
t_map_percent_population_nonwhite <- 
    tm_shape(water_systems_sac_demographics %>% 
                 mutate(percent_est_non_white = 100 - percent_est_white, # compute non-white portion of population for each water system
                        population_est_non_white = population_est_total - population_est_white # compute total non-white population for each water system
                 ) 
    ) +
    tm_polygons(col = 'percent_est_non_white',
                palette = "Blues",
                title = 'Non-White Percent' #, legend.hist = TRUE
    ) +
    tm_bubbles(size = "population_est_non_white",
               alpha = 0.5,
               col = "green",
...
t_map_percent_nonwhite_medincome <- 
    tm_shape(water_systems_sac_demographics %>% 
                 mutate(percent_est_non_white = 100 - percent_est_white # compute non-white portion of population for each water system
                 ) 
    ) +
    ...
    tm_bubbles(size = "median_household_income_est", 
               alpha = 0.5,
               col = "green",
...
plot_results_scatter <- ggplot(data = water_systems_sac_demographics %>% 
                                   mutate(percent_est_non_white = (100 - percent_est_white)/100), # compute non-white portion of population for each water system) 
                               aes(x = percent_est_non_white, 
                                   y = median_household_income_est)) +
    geom_point() +
    # scale_fill_distiller(palette = 'Blues', 
    #                      direction = 1) +
...
population_est_weighted_manual <- interpolate_manual_pop %>% 
    pull(population_weighted) %>% 
    sum() %>% 
    round(digits = 0)
population_est_weighted_manual
```

So, as a final check, the 'manually' calculated total population for <code>`r system_check`</code> is <code>`r prettyNum(population_est_weighted_manual, big.mark = ',')`</code>, and the value obtained from the `aw_interpolate` function above was <code>`r prettyNum(population_weighted_interp, big.mark = ',')`</code>.
...
median_household_income_interp <- water_systems_sac_demographics %>% 
    filter(WATER_SY_1 == system_check) %>% 
    pull(median_household_income_est)
```

In this case, we'll check the calculation for the water system: <code>`r system_check`</code>. The estimated median household income for this system using the areal interpolation above (@sec-areal-interp) was: <code>`r paste0('$', prettyNum(median_household_income_interp, big.mark = ','))`</code>.
...
```{r}
interpolate_manual_hh_inc <- interpolate_manual_hh_inc %>% 
    mutate(med_hh_inc_weighted = median_household_income * weight)
...
```{r}
# this should be the same as the interpolated result
median_household_income_est_weighted_manual <- interpolate_manual_hh_inc %>% 
    pull(med_hh_inc_weighted) %>% 
    sum() %>% 
    round(digits = 0)
median_household_income_est_weighted_manual
```

So, as a final check, the manually calculated median household income for <code>`r system_check`</code> is <code>`r paste0('$', prettyNum(median_household_income_est_weighted_manual, big.mark = ','))`</code>, and the value obtained from the `aw_interpolate` function was <code>`r paste0('$', prettyNum(median_household_income_interp, big.mark = ','))`</code>.
...
```{r}
# this should be the same as the interpolated result
population_est_weighted_manual_zero <- interpolate_manual_zero %>% 
    pull(population_weighted) %>% 
    sum() 
population_est_weighted_manual_zero
```

In this case, the (un-rounded) 'manually' calculated total population for <code>`r system_check_zero`</code> is <code>`r prettyNum(population_est_weighted_manual_zero, big.mark = ',')`</code> (for reference, the rounded value obtained from the `aw_interpolate` function was <code>`r prettyNum(population_weighted_interp_zero, big.mark = ',')`)</code>. From the @fig-manual-zero-pop above, you can see that the service area reported for this system is very small, only covering a small fraction of a single census unit, resulting in a population estimate that is very low. In this case, it could be that the system area was drawn incorrectly (i.e., maybe it doesn't really depict the entire service area), in which case the reported service area should be revised. Or, it's possible that the population within the given census unit is very un-evenly distributed and instead there's a relatively high density population cluster in the depicted service area, in which case a more sophisticated method than an area-weighted average should be used (e.g., maybe consider the density of buildings, roads, and/or other features associated with inhabited areas).
...
results_interp_popwt <- interpolate_pw(from = census_data_filter %>% 
    ...
    rename(median_household_income_est_popwt = median_household_income) # rename results field
```
...
@fig-pct-nonwhite-med-inc-compare shows a comparison (note that each plot only includes systems that have a population weighted result, so the results look a little different than the scatter plot above in @fig-pct-nonwhite-med-inc-scatter showing all systems). Also, note that these figures may not be that helpful by themselves for this example, but just provide an example of how different approaches could be compared to check whether or not the results are sensitive to the methodology used in any significant way.

```{r}
#| label: fig-pct-nonwhite-med-inc-compare
#| fig-cap: "Relationship between Non-White population and median household income for water systems in the Sacramento County Area, using an area-weighted (left, blue) versus population-weighted approach (right, orange) approach to deterime median household income"
#| fig-alt: Side by side scatter plots... (in progress)
#| warning: false
#| message: false

# Plot area weighted results
plot_results_scatter_areawt <- ggplot(data = water_systems_sac_demographics %>% 
                                           mutate(percent_est_non_white = (100 - percent_est_white)/100) %>% # compute non-white portion of population for each water system) 
                                           left_join(results_interp_popwt %>% st_drop_geometry(), 
                                                     by = 'WATER_SY_1') %>% # join the population weighted estimate
                                          filter(!is.na(median_household_income_est_popwt)), # drop results for systems where the pop weighted result is missing, for consistency
                                       aes(x = percent_est_non_white, 
                                           y = median_household_income_est)) +
    ...
    labs(title = NULL, 
         ...
         y = 'Median Household Income (Area Weighted)') +
    scale_y_continuous(labels = label_dollar(), limits = c(0, 160000)) +
    scale_x_continuous(labels = label_percent())



# Plot population weighted results
plot_results_scatter_popwt <- ggplot(data = water_systems_sac_demographics %>% 
                                           mutate(percent_est_non_white = (100 - percent_est_white)/100) %>% # compute non-white portion of population for each water system) 
                                           left_join(results_interp_popwt %>% st_drop_geometry(), 
                                                     by = 'WATER_SY_1'), # join the population weighted estimate 
                                       aes(x = percent_est_non_white, 
                                           y = median_household_income_est_popwt)) +
    geom_smooth(method = 'lm', color = 'orangered4') +    
    geom_point(color = 'orangered') +
    labs(title = NULL, 
         caption = paste0('Data from ', acs_year, ' 5-year ACS'),
         x = 'Non-White Population',
         y = 'Median Household Income (Population Weighted)') +
    scale_y_continuous(labels = label_dollar(), limits = c(0, 160000)) +
    scale_x_continuous(labels = label_percent())

# combine the two plots (side by side)
plot_results_scatter_areawt + plot_results_scatter_popwt
```

@fig-method-compare-scatter shows a more direct comparison of the population weighted approach with the original area-weighted approach.

```{r}
#| label: fig-method-compare-scatter
#| fig-cap: "Relationship between median household income for water systems in the Sacramento County Area estimated from U.S. Census Bureau data using an area-weighted versus population-weighted estimate"
#| fig-alt: Scatterplot showing median household income for Sacramento County area water systems estimated from census data, with results using an area-weighted estimate on the x axis and a population-weighted estimate on the y axis.
#| warning: false
#| message: false
#| 
plot_results_scatter_compare <- ggplot(data = water_systems_sac_demographics %>% 
                                           left_join(results_interp_popwt %>% st_drop_geometry(), 
                                                     by = 'WATER_SY_1'), # join the population weighted estimate 
                                       aes(x = median_household_income_est, 
                                           y = median_household_income_est_popwt)) +
    geom_abline(slope = 1, intercept = 0, color = 'black', linetype = 'dashed', size = 0.6, alpha = 0.8) + 
    geom_point(alpha = 0.5, size = 2) +
    labs(title = NULL, 
         caption = paste0('Data from ', acs_year, ' 5-year ACS'),
         x = 'Median Household Income (Area Weighted)',
         y = 'Median Household Income (Population Weighted)') +
    scale_y_continuous(labels = label_dollar(), limits = c(0, 160000)) +
    scale_x_continuous(labels = label_dollar(), limits = c(0, 160000))

plot_results_scatter_compare
```
