...
Finally, just as a check, add a column (which we'll call `pop_pct_difference`) to compute the difference between the estimated total population (in the `pop_est_total` field) and the total population listed in the `POPULATION` field (the reported value from the source water system dataset).
...
hash_file <- here('02_data_results', 
                  'dataset_hash.csv')
...
results_interp_popwt <- interpolate_pw(from = census_data_filter %>% 
                                       ...
                                       weight_column = 'pop_total') %>% 
    rename(med_hh_income_est_popwt = med_hh_income) # rename results field
```
...
Note that not all water systems get an estimated value using this method (even if `NA` values are removed from the source data) -- a total of `r sum(is.na(results_interp_popwt$med_hh_income))` systems are missing results with this method, which tend to be the smallest water systems.
...
plot_results_scatter_areawt <- ggplot(data = water_systems_sac_demographics %>% 
                                          ...
                                          filter(!is.na(med_hh_income_est_popwt)), # drop results for systems where the pop weighted result is missing, for consistency
                                       aes(x = pct_est_non_white, 
                                           y = med_hh_income_est)) +
