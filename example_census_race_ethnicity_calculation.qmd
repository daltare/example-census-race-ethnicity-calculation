...
It's also useful to save the results to a spatial file format, which includes both the geospatial information about each feature along with the tabular (attribute) data associated with the feature.

The chunk of code below, which is hidden by default, just tests to see whether the spatial dataset has been changed since the previous version was saved. In general this is probably not needed for a typical workflow and can be ignored for most use cases -- it is just used here to make re-rendering of this document more efficient.

```{r}
#| code-fold: true

# compute hash for spatial dataset (i.e., a unique identifier for the dataset), and compare against previous version

## define file that stores hash (unique identifier for dataset)
hash_file_sf <- here('02_data_results', 
                     'file_hash_sf.txt')

## compute hash (unique identifier for dataset)
hash_sf_current <- digest(water_systems_sac_demographics, 
                          algo = 'md5')

## get the previous hash from file (if it exists), else create a new file to store the hash
if (file.exists(hash_file_sf)) {
    hash_sf_previous <- read_lines(file = hash_file_sf)
    # check that a value exists for previous hash
    hash_sf_previous <- ifelse(is_empty(hash_sf_previous), 
                               'missing', 
                               hash_sf_previous)
} else {
    file.create(hash_file_sf)
    hash_sf_previous <- 'missing'
}

## write new hash to file
write_lines(x = hash_sf_current, 
            file = hash_file_sf, 
            append = FALSE)

## if new hash is different from previous hash, update the output file (i.e., write a new version of the file)
file_update_sf <- hash_sf_current != hash_sf_previous
```
